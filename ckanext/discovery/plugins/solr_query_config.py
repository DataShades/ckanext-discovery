        ...
        for key, value in config.items():
            if key.startswith(self.DEFAULT_PREFIX):
                ...
                try:
                    log.debug(
                        (
                            'Search parameter "{}" is already set to ' + '"{}"'
                        ).format(key, params[key])
                    )
                except KeyError:
                    log.debug(
                        (
                            'Setting search parameter "{}" to default '
                            + 'value "{}" (previously not set)'
                        ).format(key, value)
                    )
                    params[key] = value
                ...
                log.debug(
                    (
                        'Setting search parameter "{}" to "{}" (previous '
                        + 'value "{}")'
                    ).format(key, value, params.get(key, ""))
                )
                params[key] = value
        return params
