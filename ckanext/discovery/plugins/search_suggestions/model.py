...
class SearchTerm(Base):
    ...
    __table__args = (
        Index(
            "discovery_searchterm_term_tsvector_idx",
            "term_tsvector",
    ...
    def by_prefix(cls, prefix):
        ...
        return cls.filter(cls.term_tsvector.op("@@")(tsquery))
...
""".format(
        table=SearchTerm.__tablename__
    )
)
event.listen(
    SearchTerm.__table__,
    "after_create",
...
class CoOccurrence(Base):
    ...
    term1_id = Column(
        types.Integer,
        ForeignKey(SearchTerm.id, ondelete="CASCADE", onupdate="CASCADE"),
        nullable=False,
        primary_key=True,
    )
    ...
    def __repr__(self):
        r = '<{} "{}", "{}">'.format(
            self.__class__.__name__, self.term1.term, self.term2.term
        )
