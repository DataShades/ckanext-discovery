...
class SearchQuery(object):
    ...
    def __init__(self, query_string):
        ...
        if context_words:
            self.context_terms = set(
                SearchTerm.filter(SearchTerm.term.in_(context_words))
            )
        else:
            self.context_terms = set()
    ...
    def store(self):
        ...
        terms = sorted(
            (SearchTerm.get_or_create(term=t) for t in self.words),
            key=lambda t: t.term,
        )
...
class SearchSuggestionsPlugin(plugins.SingletonPlugin):
    ...
    def after_search(self, search_results, search_params):
        ...
        if not toolkit.asbool(
            get_config("search_suggestions.store_queries", True)
        ):
